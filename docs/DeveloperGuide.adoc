...
.Nearby Command Sequence Diagram
...
.Nearby Command Sequence Diagram 2
...
* Check whether the first prefix is `orderType` prefix.
...
to run the execute the import Command, this is will be shown at the <<Execution of Import Command Sequence Diagram, Execution of Import Command Sequence Diagram>>.

==== Execution of Import Command Sequence Diagram

.Import Command Sequence Diagram 2
...
* *Specfic search* will search orders based on the given `<<command_prefix, ORDER_ATTRIBUTE_PREFIX>>`.

==== Class Diagram for Search feature:

.Search Command Class Diagram
image::SearchCommandClassDiagram.svg[]
...
.. Test case: `clear` `-r` `-r` +
   Expected: Invalid command input, as duplicate flag is not allowed. +
   Error details shown in the response message. A help message displayed for the user to type the correct command. Status bar remains unchanged
.. Test case: `clear` `-r-f`+
